...
void Ball::checkCollisionWithPole() {
    ...
    if(aboveRim) {
        ...
        else if(bBall.x <= pole->getRim().x + pole->getRim().w - 13 && bBall.x + bTexture.getWidth() >= pole->getRim().x + pole->getRim().w && bBall.y <= pole->getRim().y) {
            printf("RIGHT SIDE\n", pole->getRim().x);
            checkCollision(pole->getRim().x + pole->getRim().w - 13, pole->getRim().y);
        }
        else if(bBall.x + bTexture.getWidth()/2.5 <= pole->getRim().x && bBall.x + bTexture.getWidth() >= pole->getRim().x && bBall.y <= pole->getRim().y ) {
            checkCollision(0, pole->getRim().y);
        }
        else if(bBall.x + bTexture.getWidth() >= pole->getRim().x + pole->getRim().w) {
            checkCollision(pole->getBoard().x);
        }
    }
